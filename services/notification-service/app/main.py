...
settings=get_settings(); app=FastAPI(title='AstraCart Notification Service', version='2.0.0')
...
async def unhandled(_, exc: Exception): return JSONResponse(status_code=500, content=fail('Internal server error', str(exc)))
@app.on_event('startup')
def startup(): Base.metadata.create_all(bind=engine, checkfirst=True)
@app.get('/health')
def health(): return {'success': True, 'message':'notification-service healthy','data':{'service':settings.service_name}}
